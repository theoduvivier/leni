...
import { Home, CalendarDays, FileText, Settings, Plus, LogOut } from 'lucide-react'

const tabs = [
  { href: '/', label: 'Accueil', icon: Home, mobile: true },
  { href: '/calendrier', label: 'Calendrier', icon: CalendarDays, mobile: true },
  { href: '/posts', label: 'Posts', icon: FileText, mobile: true },
  { href: '/contexte', label: 'Contexte', icon: Settings, mobile: true },
]
...
export function AppNav() {
  ...
  function emitCompose() {
    window.dispatchEvent(new CustomEvent('leni:compose'))
  }
    ...
    <>
      ...
      <aside className="hidden md:flex fixed left-0 top-0 z-50 h-dvh w-60 flex-col border-r border-white/[0.06] bg-[hsl(228,30%,6%)]/80 backdrop-blur-xl">
        ...
        <div className="px-6 pt-8 pb-6">
          <div className="flex items-center gap-3">
            <div className="relative h-14 w-14 shrink-0">
              <div className="absolute inset-0 animate-blob rounded-[30%_70%_70%_30%/30%_30%_70%_70%] bg-gradient-to-br from-accent-blue via-accent-teal to-accent-pink opacity-80 blur-[2px]" />
              <div className="absolute inset-[3px] animate-blob-reverse rounded-[60%_40%_30%_70%/60%_30%_70%_40%] bg-gradient-to-tr from-accent-pink via-accent-blue to-accent-teal opacity-90 blur-[2px]" />
            </div>
            <div>
        ...
        <div className="px-4 pb-2">
          <button onClick={emitCompose} className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-accent-blue to-[#5b8aff] py-2.5 text-[13px] font-bold text-white shadow-lg shadow-accent-blue/25 transition-all duration-200 hover:shadow-accent-blue/40 hover:brightness-110 active:scale-[0.98]">
          ...
          {tabs.filter((t) => t.mobile).map((tab, i) => {
            ...
            return (
              <span key={tab.href} className="contents">
                {i === 2 && (
                  <button onClick={emitCompose} className="flex h-11 w-11 items-center justify-center rounded-full -mt-5 bg-gradient-to-br from-accent-blue to-accent-teal text-white shadow-lg shadow-accent-blue/30 transition-transform active:scale-90">
